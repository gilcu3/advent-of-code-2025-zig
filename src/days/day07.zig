...
fn Day07() type {
    return struct {
        ...
        fn part1(self: Self) ![]const u8 {
            ...
            for (0..self.words.len - 1) |i| {
                for (0..m) |j| {
                    if (self.words[i][j] == 'S') {
                        if (self.words[i + 1][j] == '^') {
                            if (j + 1 < m) self.words[i + 1][j + 1] = 'S';
                            if (j > 0) self.words[i + 1][j - 1] = 'S';
                            ans += 1;
                        } else {
                            self.words[i + 1][j] = 'S';
                        }
        ...
        fn part2(self: Self) ![]const u8 {
            ...
            const m = self.words[0].len;
            var ts = try self.allocator.alloc([]u64, n);
            for (0..n) |i| {
                ts[i] = try self.allocator.alloc(u64, m);
                for (0..m) |j| {
                    ts[i][j] = 0;
                }
            }
...
pub fn run(_: std.mem.Allocator, is_run: bool) ![3]u64 {
    ...
    const puzzle = try Day07().init(input);
